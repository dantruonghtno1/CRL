...
class Moment:
    ...
    def update(self, ind, feature, hidden, init=False):
        self.features[ind] = feature
        self.hiddens[ind] = hidden
    ...
    def init_moment(self, args, encoder, datasets, is_memory=False):
        ...
        if not is_memory:
            ...
            self.hiddens = torch.zeros(datalen, args.encoder_output_size).cuda()
            ...
            for step, batch_data in enumerate(td):
                ...
                self.update(ind, reps.detach(), hiddens.detach())
                lbs.append(labels)
            lbs = torch.cat(lbs)
